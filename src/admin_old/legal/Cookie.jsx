    ...
    const handleSubmit = async (e) => {
        ...
        if (formValid) {
            try {
                ...
                const response = await axios.post('https://bdlearningpoint.com/backend/public/api/admin/cookie-policy', formData,
                {
                    headers: {
    ...
    useEffect(() => {
        ...
        if(token){
            const fetchData = async () => {
                try {
                    const response = await axios.get('https://bdlearningpoint.com/backend/public/api/admin/cookie-policy',
                    {
                      headers: {
                        Authorization: 'Bearer ' + token
                      }
    ...
    return (
        ...
        <div className='my-12 h-screen'>
            <div className="relative overflow-x-auto shadow-md sm:rounded-lg section-bg p-2">
                {IsSuccess && <p className='text-green-800 p-5 font-bold'>Cookie Policy Updated Succesulful</p>}
