...
const UpdateSubcategory = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (Object.keys(validationErrors).length === 0) {
            try {
                ...
                const response = await axios.post(`https://bdlearningpoint.com/backend/public/api/admin/subcategory/${params.upscatid}`, formData,
                    {
                        headers: {
    ...
    useEffect(() => {
        ...
        if (token) {
            const fetchData = async () => {
                try {
                    const response = await axios.get(`https://bdlearningpoint.com/backend/public/api/admin/subcategory/${params.upscatid}`,
                        {
                            headers: {
                                Authorization: 'Bearer ' + token
                            }
    ...
    if (!data) {
        return <Pageloader title="Update SubCategory" />;
    }

    const { title, subcategory } = data;


    return (
        <>
            <Breadcrumb title="Update SubCategory" />
                ...
                <div className="relative overflow-x-auto shadow-md sm:rounded-lg section-bg p-2">
                    <form className='p-5' onSubmit={handleSubmit}>
                        ...
                        <div className="grid md:grid-cols-2 md:gap-6">
                            {
                                IsSuccess ? (<h4 className='text-green-500 font-bold pb-8'>Sub Category Update Successfully.</h4>) : ''
                            }
                        </div>
                        <div className="grid md:grid-cols-2 md:gap-6">
                            <div className="relative z-0 w-full mb-12 group">
                                <input type="text" onChange={(e) => setFormData({ ...formData, name: e.target.value })} defaultValue={formData.name ? formData.name : subcategory.name} name="floating_first_name" id="floating_first_name" className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600" placeholder=" " required />
                                <label htmlFor="floating_first_name" className="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Sub Category Name</label>
                                {errors.title && <span className="text-red-500">{errors.name}</span>}
                            </div>
                            <div className="relative z-0 w-full mb-12 group">
                                <input type="text" onChange={(e) => setFormData({ ...formData, category_id: e.target.value })} defaultValue={formData.category_id ? formData.category_id : subcategory.category_id} name="floating_first_name" id="floating_first_name" className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600" placeholder=" " required />
