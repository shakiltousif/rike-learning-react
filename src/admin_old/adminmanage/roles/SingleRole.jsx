...
const AddRole = () => {
  ...
  const { isLoading } = useQuery(
    ...
    () => getAdminSingleRole(roleId),
    {
      onSuccess: (response) => {
        set_name(response?.data?.data?.role?.name);
        setPermissions(response?.data?.data?.permissions);
        if (response?.data?.data?.selected_permissions) {
          if (response?.data?.data?.permissions?.length != 0) {
            const selectedPermit = response?.data?.data?.permissions
              ?.filter((permission) =>
                response?.data?.data?.selected_permissions?.includes(
                  `${permission?.id}`
                )
              )
              ?.map((permission) => permission.name);

            setSelectedPermissions(selectedPermit);
          }
        }
      },
    }
