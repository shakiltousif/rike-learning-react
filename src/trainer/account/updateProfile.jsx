...
import {
  getTrainerInfo,
  postTrainerEdit,
} from "../../hooks/trainer/trainerApi.js";
import TrainerProfileSkeleton from "../../components/Skeleton/trainer/ProfileSkeleton.jsx";
...
const SingleStudent = () => {
  ...
  const { isLoading: isSubmitLoading, mutate } = useMutation(postTrainerEdit, {
    onSuccess: () => {
  ...
  useEffect(() => {
    if (instructor?.uuid) {
      set_first_name(instructor?.first_name);
      set_last_name(instructor?.last_name);
      set_email(user?.email);
      set_phone_number(user?.mobile_number);
      set_area_code(user?.area_code);
      set_address(instructor?.address);
      set_gender(instructor?.gender);
      set_about_me(instructor?.about_me);
  ...
  if (isFetching) content = <TrainerProfileSkeleton />;
    ...
    content = (
      <form className="p-5" onSubmit={handleSubmit}>
        <div className="relative z-0 w-full mb-6 group">
          <input
            type="id"
            name="floating_account_type"
            id="floating_account_type"
