...
import { deleteCourse, getCourses } from "../../hooks/admin/courseApi";
...
const AllCourses = () => {
  ...
  const {
    ...
    error,
    refetch,
  } = useQuery("getCourses", () => getCourses({ count }));
  ...
  if (!isLoading && courses?.data?.data?.courses?.length > 0)
    content = courses?.data?.data?.courses?.map((course, key) => (
      ...
      >
        ...
        <td data-label={"Sub Title"} className="px-6 py-4">
          {course?.subtitle}
        </td>
        ...
        <td data-label={"Action"} className="px-6 py-4">
          <div className="flex items-center justify-end gap-2">
            <Link
              ...
              className="font-medium text-blue-300 underline hover:underline"
            >
              Edit
            </Link>
            <DeleteModal
