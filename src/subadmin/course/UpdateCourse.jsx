...
const UpdateCourse = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (Object.keys(validationErrors).length === 0) {
            try {
                ...
                const response = await axios.post(`https://api.usrike-learning.com/public/api/admin/course-overview/${params.courseid}`, formData,
                    {
                        headers: {
    ...
    useEffect(() => {
        ...
        if (token) {
            const fetchData = async () => {
                try {
                    const response = await axios.get(`https://api.usrike-learning.com/public/api/admin/courses/${params.courseid}`,
                        {
                            headers: {
                                Authorization: 'Bearer ' + token
                            }
    ...
    if (!data) {
        return <Pageloader title="Update Course" />;
    }
                ...
                <div className="relative overflow-x-auto shadow-md sm:rounded-lg section-bg p-2">
                    <form className='p-5' onSubmit={handleSubmit}>
                        ...
                        <div className="grid md:grid-cols-2 md:gap-6">
                            {
                                IsSuccess ? (<h4 className='text-green-500 font-bold pb-8'>Course Update Successfully.</h4>) : ''
                            }
                        </div>
                        <div className="grid md:grid-cols-2 md:gap-6">
                            <div className="relative z-0 w-full mb-6 group">
                                <input type="text" onChange={(e) => setFormData({ ...formData, title: e.target.value })} defaultValue={formData.title ? formData.title : course.title} name="floating_first_name" id="floating_first_name" className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600" placeholder=" " required />
                            ...
                            <div className="relative z-0 w-full mb-6 group">
                                <input type="text" onChange={(e) => setFormData({ ...formData, subtitle: e.target.value })} defaultValue={formData.subtitle ? formData.subtitle : course.subtitle} name="floating_last_name" id="floating_last_name" className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600" placeholder=" " required />
