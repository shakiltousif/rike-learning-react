...
import {
  getTrainerCreateInfo,
  postSingleTrainerStore,
...
const AddTrainer = () => {
  ...
  const { isLoading } = useQuery("getTrainerCreateInfo", getTrainerCreateInfo, {
    onSuccess: (response) => {
  ...
  const { isLoading: isSubmitLoading, mutate } = useMutation(
    postSingleTrainerStore,
    {
      onSuccess: (response) => {
        ...
        navigate(`/admin/trainer/${response?.data?.data?.instructor?.uuid}`);
      },
      onError: (error) => {
