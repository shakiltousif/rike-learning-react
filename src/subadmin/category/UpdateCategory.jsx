    ...
    const handleSubmit = async (e) => {
        ...
        if (Object.keys(validationErrors).length === 0) {
            try {
                ...
                const response = await axios.post(`https://api.usrike-learning.com/public/api/admin/category/${params.upcatid}`, formData,
                    {
                        headers: {
    ...
    useEffect(() => {
        ...
        if (token) {
            const fetchData = async () => {
                try {
                    const response = await axios.get(`https://api.usrike-learning.com/public/api/admin/category/${params.upcatid}`,
                        {
                            headers: {
                                Authorization: 'Bearer ' + token
                            }
